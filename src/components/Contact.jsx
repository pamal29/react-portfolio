...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-16 animate-fade-in">
      <h2 className="text-4xl font-bold text-center mb-8 text-textLight">Get In Touch</h2>

      {submitted ? (
        <div className="max-w-2xl mx-auto bg-bg-soft/50 border-2 border-neonPurple p-12 text-center rounded-2xl animate-scale-in">
          <Send className="w-16 h-16 mx-auto mb-4 text-neonPurple animate-bounce-in" />
          <h3 className="text-2xl font-bold text-textLight">Message Sent!</h3>
          <p className="text-mutedLight text-sm mt-2">I'll get back to you soon.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-5">
          <div className="relative">
            <User className="absolute left-4 top-4 w-5 h-5 text-muted" />
            <input
              type="text"
              value={formData.name}
              placeholder="Your Name"
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="w-full bg-bg-soft/50 border border-borderMuted text-textLight placeholder:text-muted
                         p-4 pl-12 rounded-xl focus:border-neonPurple outline-none transition-colors"
            />
          </div>

          <div className="relative">
            <Mail className="absolute left-4 top-4 w-5 h-5 text-muted" />
            <input
              type="email"
